...
do
{
    ...
    do
    {
        TP = ConsoleExtension.GetValidOptions("Tipo producto [P]perecedero, [N]o perecedero:", tpOptions);
    }
    ...
    do
    {
        TC = ConsoleExtension.GetValidOptions("Tipo producto [F]frio, [A]mbiente:", tcOptions);
    }
    while (!tcOptions.Any(x => x.Equals(TC, StringComparison.CurrentCultureIgnoreCase)));

    var PC = ConsoleExtension.GetInt("Periodo conservacion (dias).........:");
    var PA = ConsoleExtension.GetInt("Periodo almacenamiento (dias).......:");
...
decimal GetCostoExHibision(string? TP, string? TC, string? MA, decimal CA)
{
    if (TP!.Equals("p", StringComparison.CurrentCultureIgnoreCase))
    {
